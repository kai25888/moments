      ...
      <div class="flex flex-col gap-5">
        ...
        <div class="grid grid-cols-3 gap-3">
          ...
          <div class="rounded-2xl border border-black/5 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/5">
            ...
            <div class="mt-2 text-sm font-medium text-slate-700 dark:text-slate-200">{{ sysConfig.enableComment ? "开放互动" : "静态阅读" }}</div>
          </div>
          <div class="rounded-2xl border border-black/5 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/5">
            <div class="text-[11px] uppercase tracking-[0.24em] text-slate-400 dark:text-slate-500">Owner</div>
            <div class="mt-2 truncate text-sm font-medium text-slate-700 dark:text-slate-200">{{ currentUser.nickname || currentUser.username || "站点作者" }}</div>
          </div>
        </div>
